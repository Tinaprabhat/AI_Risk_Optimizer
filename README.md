...
│   │   ├── fix.py                 # Fix Engine endpoints
│   │   └── mirror.py              # AI Mirror endpoint
│   ├── engine/
│   │   ├── extractor.py           # 3-tier semantic extractor
│   │   ├── layers/
│   │   │   ├── l1_crawlability.py
│   │   │   ├── l2_structured_data.py
│   │   │   ├── l3_semantic_content.py
│   │   │   ├── l4_trust_signals.py
│   │   │   ├── l5_ai_protocols.py
│   │   │   ├── l6_semantic_gap.py
│   │   │   └── l7_aggregation.py
│   │   └── fix_engine.py
│   ├── llm/
│   │   ├── gemini.py              # Primary LLM client
│   │   ├── ollama.py              # Local fallback
│   │   └── template.py            # Hardcoded fallback
│   ├── db/
│   │   ├── database.py
│   │   └── models.py
...
├── frontend/
│   └── src/
│       ├── pages/                 # Landing, Onboarding, Results,
│       │                          # AIМirror, FixNow
│       └── components/
├── docker-compose.yml
├── render.yaml
...
|---|---|---|
| `POST` | `/api/audit` | Trigger a new store audit |
| `GET` | `/api/audit/{id}` | Retrieve cached audit result |
| `GET` | `/api/mirror/{id}` | AI Mirror — 3-perception comparison |
| `POST` | `/api/fix/start` | Start a fix session for a checkpoint |
| `POST` | `/api/fix/chat` | Continue a fix conversation |
| `GET` | `/health` | System health — Gemini, Ollama, DB status |
