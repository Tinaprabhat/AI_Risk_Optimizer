...
def compute_gap(
    ...
    if v2 is not None:
        ...
        print(f"  🪞 Gap IW (Intent vs Website) : {gap_IW} → {_classify_gap(gap_IW)}", flush=True)
    ...
    if v3 is not None:
        ...
        print(f"  🪞 Gap IS (Intent vs Schema)  : {gap_IS} → {_classify_gap(gap_IS)}", flush=True)
    ...
    if v2 is not None and v3 is not None:
        ...
        print(f"  🪞 Gap WS (Website vs Schema) : {gap_WS} → {_classify_gap(gap_WS)}", flush=True)
