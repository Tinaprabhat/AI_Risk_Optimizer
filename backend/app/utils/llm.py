...
def _call_gemini(prompt: str, system: str) -> Optional[str]:
    ...
    try:
        ...
        print(f"  🤖 Gemini responded ({len(text)} chars)", flush=True)
        return text
...
def _call_ollama(prompt: str, system: str) -> Optional[str]:
    ...
    try:
        ...
        print(f"  🤖 Ollama Mistral responded ({len(text)} chars)", flush=True)
        return text
    except Exception as e:
...
# The store failed these audit rules:
